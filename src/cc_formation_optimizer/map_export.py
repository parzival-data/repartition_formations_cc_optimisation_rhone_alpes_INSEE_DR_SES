...
<head>
  ...
  <style>
    ...
    * {{ box-sizing:border-box; }}
    ...
    #tileLayer img {{ position:absolute; width:256px; height:256px; opacity:.65; }}
...
<script>
let state = {{ scale:1, tx:0, ty:0, selectedSession:null, dragging:false, last:null }};
const colors = new Map(summary.map((s,i)=>[s.id_session, `hsl(${{(i*67)%360}} 65% 43%)`]));
const wrap = document.getElementById('mapWrap'), svg = document.getElementById('svgLayer'), tip = document.getElementById('tooltip');
...
function bounds(data) {{ const xs=data.map(p=>p.lon), ys=data.map(p=>p.lat); return {{ minLon:Math.min(...xs), maxLon:Math.max(...xs), minLat:Math.min(...ys), maxLat:Math.max(...ys) }}; }}
function project(p, b) {{ const w=wrap.clientWidth, h=wrap.clientHeight, pad=36; const dx=(b.maxLon-b.minLon)||1, dy=(b.maxLat-b.minLat)||1; return {{ x:(pad+(p.lon-b.minLon)/dx*(w-2*pad))*state.scale+state.tx, y:(h-pad-(p.lat-b.minLat)/dy*(h-2*pad))*state.scale+state.ty }}; }}
function render() {{
  const data = filteredPoints(); svg.innerHTML = '';
  ...
  if (document.getElementById('showLinks').checked) {{
    data.forEach(p => {{ const pivot = points.find(q=>q.code_commune===p.code_pivot); if (!pivot) return; const a=project(p,b), c=project(pivot,b); svg.insertAdjacentHTML('beforeend', `<line x1="${{a.x}}" y1="${{a.y}}" x2="${{c.x}}" y2="${{c.y}}" stroke="${{colors.get(p.id_session)}}" stroke-width="1" opacity=".35"/>`); }});
  }}
  data.forEach(p => {{ const c=project(p,b), color=colors.get(p.id_session)||'#2563eb', size=p.is_pivot?12:8, stroke=p.alert_level==='warning'?'#b45309':p.alert_level==='error'?'#b91c1c':'#fff'; const shape=p.categorie==='PC'?`<rect x="${{c.x-size/2}}" y="${{c.y-size/2}}" width="${{size}}" height="${{size}}" rx="2"`:`<circle cx="${{c.x}}" cy="${{c.y}}" r="${{size/2}}"`; svg.insertAdjacentHTML('beforeend', `${{shape}} fill="${{color}}" stroke="${{stroke}}" stroke-width="${{p.alert_level==='ok'?1.5:3}}" data-code="${{p.code_commune}}" style="cursor:pointer"/>`); }});
  [...svg.children].forEach(el => {{ el.onmousemove = e => showTip(e, points.find(p=>p.code_commune===el.dataset.code)); el.onmouseleave = hideTip; el.onclick = () => {{ const p=points.find(q=>q.code_commune===el.dataset.code); state.selectedSession=p.id_session; showDetail(p); renderTables(data); }}; }});
  renderTables(data); renderCharts();
}}
...
document.getElementById('zoomIn').onclick=()=>{{state.scale*=1.25; render();}}; document.getElementById('zoomOut').onclick=()=>{{state.scale/=1.25; render();}}; document.getElementById('resetMap').onclick=()=>{{state={{scale:1,tx:0,ty:0,selectedSession:null,dragging:false,last:null}}; render();}};
